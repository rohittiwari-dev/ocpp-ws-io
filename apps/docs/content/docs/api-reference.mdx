...
Exactly like `call()`, except it catches all errors and returns an object of `{ success: boolean, result?, error? }` rather than throwing. Useful for fire-and-forget or localized error handling without breaking loops.

```typescript
const { success, result, error } = await client.safeCall(
  "ocpp1.6",
  "ClearCache",
  {},
);
...
server.auth((accept, reject, handshake) => {
  // handshake has .identity, .pathname, .headers, etc.
...
Helper method to dispatch an outgoing RPC call to a specific connected client by its `identity`. Throws an error if the station is not connected. Matches the `client.call()` signature.

```typescript
const result = await server.sendToClient("CP-001", "ocpp1.6", "Reset", {
  type: "Soft",
});
```
...
Exactly like `sendToClient()`, but catches errors (and disconnected station errors), returning a standard `{ success: boolean, result?, error? }` structure. Highly recommended for bulk operations across your cluster.

```typescript
const { success, error } = await server.safeSendToClient(
  "CP-001",
  "ocpp1.6",
  "UpdateFirmware",
  { location: "..." },
