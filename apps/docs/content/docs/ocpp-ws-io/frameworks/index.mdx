...
`ocpp-ws-io` provides a dedicated integration for Express that seamlessly injects the OCPP context into your route handlers.

For full details and API reference, see the [Express Integration Guide](./frameworks/express).

If you want to do it manually without the native middleware:
...
`ocpp-ws-io` provides first-class native integration with NestJS, including dynamic module registration, decorator-based event routing, and robust parameter injection.

For full details, see the dedicated [NestJS Integration Guide](./frameworks/nestjs).
...
console.log("Bun Node-compat server listening on 3000");
```
...
await server.listen(3000);
console.log("Deno Node-compat server listening on 3000");
```
...
See [Browser Client](/docs/ocpp-ws-io/browser-client) for the full API reference and framework integration examples.
