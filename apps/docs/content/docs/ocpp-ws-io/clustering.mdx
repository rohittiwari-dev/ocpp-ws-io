...
telemetry.pendingMessages; // 142  — upper bound on unconsumed stream entries
telemetry.activeStreams; //  3    — streams currently polled
telemetry.streamDetails; //  { "ocpp-ws-io:ocpp:node:abc": 142, ... }
telemetry.pollErrors; //     0    — consecutive stream-read failures
telemetry.lastPollError; //  present only while pollErrors > 0
```

<Callout type="warn">
`pendingMessages` counts retained entries via `XLEN`, including ones already
consumed but not yet trimmed by `MAXLEN`. Treat it as an upper bound on backlog,
not an exact unread count.

`pollErrors` is the one to alert on: any non-zero value means cross-node
delivery *into* this node is currently failing.
</Callout>
...
// Hand the driver straight to the adapter.
const adapter = new RedisAdapter({ driver: clusterDriver });
```
...
<Callout type="warn">
**`getPresence()` is what makes routing cross-node.** The presence methods are
optional on the interface, but without `getPresence()` the server has no way to
discover which node owns an identity, so `sendToClient` can only ever reach
clients on the calling node — everything else falls through to
"Client not found". An adapter missing it logs a warning at `setAdapter()`.
</Callout>

### Presence fencing

`setPresence()` and `removePresence()` are unconditional writes, which is not
safe with more than one node. A charger that drops from node A and reconnects to
node B leaves A's teardown running afterwards, and an unconditional
`removePresence()` there deletes the entry B just wrote — the charger becomes
unroutable until the next heartbeat.

Implement these two optional methods and the server uses them instead. Both must
be **atomic** on the server side (a Lua script on Redis, a transaction
elsewhere); a read followed by a write in your adapter just moves the race:

```typescript
/** DEL the entry only if it currently names `nodeId`. */
removePresenceIfOwned?(identity: string, nodeId: string): Promise<boolean>;

/**
 * Set the entry to `nodeId` with `ttl` only if it is absent or already ours.
 * Returning false tells the server another node owns this identity and its
 * local socket is stale.
 */
claimPresence?(identity: string, nodeId: string, ttl: number): Promise<boolean>;
```

Adapters that omit them keep working — the server falls back to the
unconditional calls, with the race described above. The shipped `RedisAdapter`
and `InMemoryAdapter` both implement them.

### Delivery guarantees

The two cross-node paths are not equivalent, and the difference matters:

| Path | Transport | Guarantee |
| --- | --- | --- |
| `sendToClient`, `sendBatch` (unicast) | Redis Streams | Survives a moment when the target node is not subscribed |
| `broadcast`, `broadcastBatch` | Redis Pub/Sub | Fire-and-forget — a node not subscribed at that instant never sees it |

`broadcast()` returns a `BroadcastResult`. Its `remotePublished` flag means the
message was handed to the adapter, **not** that any other node received it —
pub/sub cannot tell you that. Only `localDelivered` and `localFailed` describe
actual delivery.
