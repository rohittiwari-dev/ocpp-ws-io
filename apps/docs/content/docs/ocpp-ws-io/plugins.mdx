...
The two overlap, and the dividing line is what you need to do to the message:

- **A plugin hook observes or vetoes.** `onBeforeSend` and `onBeforeReceive` see every
  message in both directions and can block one, but the value they return is not what
  goes on the wire — they cannot rewrite a payload. Hooks also get lifecycle events a
  middleware never sees: connect, disconnect, auth failure, error.
- **[Middleware](/docs/ocpp-ws-io/middleware) transforms.** Assigning to `ctx.params`,
  `ctx.payload`, `ctx.errorCode` or `ctx.errorDescription` changes the message that is
  actually sent or handled.

So: gate with a hook, change with middleware. A plugin can do both — it registers
middleware from its `onConnection` hook, which is exactly how the two Level 4 plugins
below work.
...
server.plugin(
  ...
  piiRedactorPlugin({ sensitiveKeys: ["idToken", "email"] }),
...
| :--- | :--- | :--- |
| `onInit` | `(server)` | Plugin is registered — **not** when the server starts listening. Runs again on `listen()` when a closed server is restarted, since `close()` has torn the plugin down. |
| `onConnection` | `(client)` | New charging station connects and is fully initialized. Also fired once per already-connected client when a plugin is registered on a running server, so its per-connection state is not missing connections it never saw open. |
| `onDisconnect` | `(client, code, reason)` | Client disconnects normally. Messages already queued on that connection's inbound chain are still delivered to `onMessage` afterwards, so a few observations can trail the disconnect. |
| `onEviction` | `(evicted, new)` | Client is forcibly disconnected because another client connected with the same identity. The replacement's `onConnection` fires **before** the evicted socket's `onDisconnect`, so key any per-connection state by the client object rather than by identity. |
| `onClosing` | `()` | Pre-shutdown hook before connections are drained. Awaited, but bounded by `pluginShutdownTimeoutMs` (default 5000) — a hook that overruns is abandoned so one plugin cannot hang shutdown. |
| `onClose` | `()` | Server shutting down. Also bounded by `pluginShutdownTimeoutMs`. A server that is later restarted gets a fresh `onInit`, so treat this as a teardown that may be followed by another setup. |
...
`ocpp-ws-io` enforces strict validation checks when plugins are initialized to prevent silent failures and ensure system resilience:

### Strict Validation & Error Handling
- **PII Redaction Guard**: The `piiRedactorPlugin` checks options at startup and will throw an informative error if `options.sensitiveKeys` is missing or empty. This prevents zero-key PII redaction which would otherwise fail silently without masking anything.
- **Graceful Redis-backed Plugin Handling**: If a Redis-backed plugin (like `messageDedupPlugin` or `replayBufferPlugin`) is registered without passing a required `redis` client instance, the system automatically logs a warning and skips the initialization of that specific plugin rather than throwing a hard crash error. This guarantees that the rest of your server's plugin chain (e.g., logging, metrics) continues to function normally.
...
| :--- | :--- | :--- | :--- |
| `sensitiveKeys` | `string[]` | **required** | Keys to redact (matched recursively, at any depth). No default — you must list them explicitly. |
| `replacement` | `string` | `"***REDACTED***"` | Replacement value for redacted keys. |
| `incoming` | `boolean` | `true` | Redact inbound message payloads. |
| `outgoing` | `boolean` | `false` | Redact outbound message payloads. Off by default — see the warning below. |

```typescript
import { piiRedactorPlugin } from "ocpp-ws-io/plugins";

server.plugin(piiRedactorPlugin({
  sensitiveKeys: ["password", "authorizationKey", "email", "phoneNumber"],
  replacement: "***",
}));
```

<Callout type="warn">
  **`sensitiveKeys` is required — there is no default.** This is deliberate:
  redaction **mutates the live payload, not just logs**, so a key redacted on an
  **incoming** message is also hidden from your handlers. If you list `idTag` and
  your handlers authorize by it (`Authorize`, `StartTransaction`), they'll receive
  the placeholder instead of the real value — set `incoming: false` in that case,
  or don't include `idTag`. Redaction is also **irreversible**; forward the
  original to a secure store *before* this plugin runs (a Level 1 plugin or a
  separate `onMessage` hook) if you need it.
</Callout>

<Callout type="warn">
  **`outgoing` defaults to `false`, and turning it on corrupts commands.** An
  outbound message is built from the payload *after* middleware runs, so redaction
  applies to the wire, not to a logging copy. With `idTag` among `sensitiveKeys`,
  every `RemoteStartTransaction` reaches the charger carrying `"***REDACTED***"` as
  the tag and the charger tries to authorize that string — remote start fails, and
  it presents as a charger fault. Only enable `outgoing` when the keys you are
  redacting are ones the charge point does not need to act on.
</Callout>
...
| `events` | `KafkaEvent[]` | `["connect", "disconnect", "message", "security"]` | Which events to publish. Note the default is **not** all of them &mdash; `"auth_failed"` and `"eviction"` only publish if you list them explicitly. |
