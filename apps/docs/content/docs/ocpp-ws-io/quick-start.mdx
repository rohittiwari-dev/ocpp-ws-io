...
// Do this BEFORE connect(): the socket starts dispatching the moment it opens,
// and a CALL with no registered handler is answered with a NotImplemented
// CALLERROR. Registering after `await client.connect()` is already too late.
...
For larger CSMS projects, writing all logic inside a single `server.on('client')` handler isn't scalable. Check out **[Connection Routing](/docs/ocpp-ws-io/routing)** to learn how to compartmentalize distinct domains using the Express-like `OCPPRouter`!
