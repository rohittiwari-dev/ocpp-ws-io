...
// Registering before connect() is clearest. A CALL that arrives during startup
// waits briefly for its handler instead of being rejected, so registering just
// after connect() also works — see ClientOptions.handlerGraceMs.
