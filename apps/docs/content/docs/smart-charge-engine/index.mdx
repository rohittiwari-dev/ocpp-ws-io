...
Persist active sessions (e.g. to Redis/DB) and restore them after a process restart
so allocations survive a redeploy.
...
including when per-session `minChargeRateKw` floors are in play.

If the floors add up to more than the grid can supply, allocations are scaled down
to fit and a `gridOverCommitted` event fires:

```typescript
engine.on("gridOverCommitted", (info) => {
  console.warn(`Over-subscribed: wanted ${info.requestedKw}kW, grid is ${info.effectiveGridLimitKw}kW`);
  if (!info.feasible) {
...
<Callout type="info">
  This guard runs after **every** strategy — built-in or custom — so a custom
  strategy can never push the total above the grid limit.
</Callout>
...
| `phases` | `1 \| 2 \| 3` | `3` | AC phase count (2 = split-phase) |
...
| `phases` | `1 \| 2 \| 3` | site default | Phase count for connector (2 = split-phase) |
...
| `updateSession(txId, patch)` | Update a session in place (no clear/re-add side effects) |
...
| `dispatch()` | Calculate + dispatch to all active sessions (overlapping calls coalesce) |
...
| `loadSnapshot(sessions, opts?)` | Restore sessions from a snapshot (validated, atomic) |
...
| `sessionUpdated` | `ActiveSession` | Session updated via `updateSession()` |
...
| `gridOverCommitted` | `GridOverCommitInfo` | Min-rate floors exceed grid; allocations scaled (incl. `starvedSessions`) |
