...
- **Latency**: Round-trip min / avg / p50 / p95 / p99 / max using sub-millisecond `performance.now()` precision.
- **Connection Time**: WebSocket handshake + BootNotification round-trip.
