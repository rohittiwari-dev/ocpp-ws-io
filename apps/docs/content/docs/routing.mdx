...
## Execution Scope (Clarification on Route vs Global)

A very important concept in `ocpp-ws-io` is understanding **where** middlewares and auth callbacks execute.

### Route-level Scoping

If you define `router.use()` and `router.auth()` on a specific `.route()`, they **only** execute for clients connecting to that exact URL path. They **do not** execute globally across your server.
...
If you want a middleware to run for **every** connection to your server regardless of the route (e.g., a global rate limiter), you attach it directly to the `server` instance _without_ calling `.route()`.

```typescript
// This runs for EVERY connection globally
