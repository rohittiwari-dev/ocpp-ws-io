...
import { Queue } from "../src/browser/queue.js";
...
describe("Browser Queue", () => {
  ...
  it("should enforce minimum concurrency of 1 in setConcurrency", () => {
    const q = new Queue(3);
    q.setConcurrency(0);
    expect(q.concurrency).toBe(1);
    q.setConcurrency(-2);
  ...
  it("should drain pending tasks when concurrency increases", async () => {
    const q = new Queue(1);
    let resolveFirst!: () => void;
    ...
    // Initially only p1 is running
    expect(q.running).toBe(1);
    expect(q.pending).toBe(2);

    // Increase concurrency → should drain pending
    q.setConcurrency(3);
    resolveFirst();
